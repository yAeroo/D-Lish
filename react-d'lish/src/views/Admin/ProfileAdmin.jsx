...
export default function ProfileAdmin() {
  ...
  // Botón de carga ======
  const [saveBttn, setSaveBttn] = useState(t("actions.save-changes"));
  const [statusBttn, setStatusBttn] = useState(true);
  const loadBttn = () => {
    setSaveBttn(<ImSpinner className="loading-icon" />);
    setStatusBttn(true);
  };
  const resetLoadBttn = () => {
  ...
  const cafeteriaNameRef = createRef();
  const mainPublicRef = createRef();
  const contactEmailRef = createRef();
  const imgWallRef = createRef();
  ...
  const settings = async (dataPass) => {
    ...
    try {
      ...
      if (data.changes.statusChanges) {
        NotiExito();
        setErrores([]);
        data.changes.nameFlag
          ? ((inputs.cafeteriaName.placeholder = dataPass.name),
            (inputs.cafeteriaName.value = ""))
          : "";
        data.changes.mainPublic
          ? ((inputs.mainPublic.placeholder = dataPass.mainPublic),
            (inputs.mainPublic.value = ""))
          : "";
        data.changes.contactEmailFlag
          ? ((inputs.contactEmail.placeholder = dataPass.contactEmail),
            (inputs.contactEmail.value = ""))
          : "";
      ...
      error.response.data.changes.nameFlag
        ? (inputs.cafeteriaName.value = "")
        : "";
      error.response.data.changes.mainPublic
        ? (inputs.mainPublic.value = "")
        : "";
      error.response.data.changes.contactEmail
        ? (inputs.contactEmail.value = "")
        : "";
    }
  };
  ...
  return (
    <>
      <div
        id="main-container"
        className="lg:ml-[10rem] sm:ml-[5rem] ml-[2rem] h-[100%] animate-fade animate-duration-500"
      >
        <div className="w-[90%] mb-14">
          ...
          >
            <div id="errores" className="px-12 mb-7">
              ...
              {errores
                ? errores.map((error, i) => <Alert key={i}>{error}</Alert>)
                : ""}
            </div>

            <div
              id="grid-cont"
              className="grid md:grid-cols-2 md:grid-rows-1 gap-y-4 gap-x-2"
            >
              {/* Nombre de la cafeteria */}
              <div className="form-control w-full inline-block md:text-center">
                <label className="label">
                  <span className="label-text text-[17px] text-success font-title font-bold ml-4">
                    {t("admin.cafe-name")}
                  </span>
                </label>
                <input
                  ...
                  id="cafeteriaName"
                  ref={cafeteriaNameRef}
                  className="input bg-[#eaecef] text-[#2e2e2f] w-full md:max-w-md"
              ...
              <div className="form-control w-full inline-block md:text-center">
                ...
                <input
                  type="text"
                  placeholder={cafeteria?.mainPublic}
                  id="mainPublic"
                  ref={mainPublicRef}
                  className="input bg-[#eaecef] text-[#2e2e2f] w-full md:max-w-md"
                  onChange={handleInputChange}
              ...
              <div className="form-control w-full inline-block md:text-center">
                <div className="center">
                  <label className="label">
                    <span className="label-text text-[17px] text-success font-title font-bold ml-4">
                      {t("admin.contac-email")}
                    </span>
                  </label>
                  <input
                    type="email"
                    placeholder={cafeteria?.contactEmail}
                    id="contactEmail"
                    ref={contactEmailRef}
                    className="input bg-[#eaecef] text-[#2e2e2f] w-full md:max-w-md"
                    onChange={handleInputChange}
              ...
              {/* Idioma */}
              <div className="form-control w-full inline-block md:text-center p-3">
                <li className="li-elem !mb-4 text-primary">
              ...
              <div className="form-control w-full md:col-span-2 items-center">
                ...
                <label
                  htmlFor="image"
                  id="div-file"
                  className="text-gray-600 w-10/12 sm:w-7/12 md:w-1/2 px-5 py-12 my-5 cursor-pointer flex justify-center items-center rounded-lg border-dashed border-2 !bg-[#e6e6e6] border-[#4c4c4c]"
                >
                  <input
                    type="file"
                    name="img_wall"
                  ...
                  <div className="flex justify-center flex-col items-center w-full">
                    <div
                      id="img-cont"
                      className="h-24 w-44 flex items-center justify-center mb-3"
                    >
                    ...
                    <h3 id="original-text" className=" font-semibold">
                      {t("info.upload-img")}
                    </h3>

                    <h3
                      id="file-upload-filename"
                      className="text-ellipsis text-center hidden overflow-hidden font-semibold whitespace-nowrap w-full"
                    >
                    ...
                    {error && (
                      <div className="flex font-semibold text-center animate-shake animate-duration-200 text-error">
                        {" "}
                        <BiSolidError className="self-center mr-2" /> {error}
                      </div>
                    )}
                  </div>
                </label>
